...
# input /home/guzuxing/Desktop/jabref/guzuxing/verified_log_gzx.txt /home/guzuxing/Desktop/jabref_git_revision
import os
import sys
import time
...
def getCloneDetectionResult(singlePathNames, gitPath):
    ...
    if True:
        for item in singlePathNames:
            ...
            command = '%s -minblock %d -minclone %d -log %s -informat single -input %s -language %s -outformat %s -output %s' % (
                TOOLPATH, MINBLOCK, MINCLONE, logPath, filePath, LANGUAGE, OUTFORMAT, output)
    ...
    for item in singlePathNames:
        ...
        output = dir + item + "_clone_detection_result.txt"
        index += 1
        print('extract verified clone group from bug_clone_detection_result file in', item,
              '[%d of %d]' % (index, total))
...
# singlePathNames = ['13_464ba52a83620de']
